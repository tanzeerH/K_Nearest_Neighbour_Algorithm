...
public class TrainStructure {
	...
	/*
	 * private HashMap<String, Integer> wordMap = new HashMap<String,
	 * Integer>(); private ArrayList<ArrayList<Integer>> fetvectorList = new
	 * ArrayList<ArrayList<Integer>>(); private ArrayList<String> wordList = new
	 * ArrayList<String>(); private ArrayList<String> docTopicList = new
	 * ArrayList<String>();
	 */
	...
	public void setWordMap(HashMap<String, ArrayList<Integer>> wordMap) {
		this.wordMap = wordMap;
	}

	/*
	 * public HashMap<String, Integer> getWordMap() { return wordMap; }
	 * 
	 * public void setWordMap(HashMap<String, Integer> wordMap) { this.wordMap =
	 * wordMap; }
	 * 
	 * public ArrayList<ArrayList<Integer>> getFetvectorList() { return
	 * fetvectorList; }
	 * 
	 ...
	 * 
	 * public void setWordList(ArrayList<String> wordList) { this.wordList =
	 * wordList; }
	 * 
	 * public ArrayList<String> getDocTopicList() { return docTopicList; }
	 * 
	...
	private void readFileForCount() {
		...
		System.out.println("TOTAL DOC " + TOTAL_DOC + " DOC topics size: "
				+ docTopicList.size());
	...
	private void readFileForParsing() {
				...
				if (sCurrentLine.length() != 0) {
					...
					for (int i = 0; i < values.length; i++) {
						...
						if (wordMap.containsKey(values[i])) {
							if (!wordMap.get(values[i]).contains(docIndex)) {
								wordMap.get(values[i]).add(docIndex);
							}

						} else {
							ArrayList<Integer> list=new ArrayList<Integer>();
							wordMap.put(values[i],list);
							wordMap.get(values[i]).add(docIndex);

						}

						/*
						 * if (wordMap.containsKey(values[i])) { int wordIndex =
						 * wordMap.get(values[i]); ArrayList<Integer> vList =
						 * fetvectorList .get(wordIndex); vList.set(docIndex,
						 * vList.get(docIndex) + 1); } else { int size =
						 * wordMap.size(); wordMap.put(values[i], size);
						 * wordList.add(values[i]); ArrayList<Integer> vList =
						 * getArraListsWithInitialization(); vList.set(docIndex,
						 * vList.get(docIndex) + 1); fetvectorList.add(vList); }
						 */
					}
				}
		...
		System.out.println("DocWordList Size: " + docWordList.size()+"  wordmap size: "+ wordMap.size());

		/*
		 * for(int i=0;i<100;i++) { System.out.println("hash size:"+
		 * hashList.get(i).size() + " wordList size: "+
		 * docWordList.get(i).size()); }
		 */
